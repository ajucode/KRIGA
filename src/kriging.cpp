...
krig::KrigError::KrigError( const Exep e, const GA_array& array ) :
    _error( e ),
    _garray( array ) {
    }

const char* krig::KrigError::what() const throw () {
    std::string message;
    switch ( _error ) {
    case type:
...
std::ostream& operator << ( std::ostream& out, const krig::KrigError& Error  ) {
    out << "Error:" << std::endl;
...
*/
GA::GlobalArray* krig::covariance( GA::GlobalArray* X, 
				   GA::GlobalArray* w,
	...
	if ( dimsX[1] == dimsw[0] ) {
	    ...
	    GA_array galist = { X, w };
	    throw KrigError( krig::KrigError::dimension, galist );
	    return nullptr;
	}
    }
    else {
	GA_array galist = { X, w };
	throw KrigError( krig::KrigError::type, galist );
	return nullptr;
    }
}


GA::GlobalArray* krig::covariance_prediction( GA::GlobalArray* x0, 
					      GA::GlobalArray* X, 
					      GA::GlobalArray* w,
					      double (* kernel)( const double& ) ) {
    int n = 1;
...
/*__________________________________________________________________________________________________
  Simple Kriging
*/
krig::KrigResult krig::simple_kriging( GA::GlobalArray* x0, 
				       GA::GlobalArray* X,
				       GA::GlobalArray* Z,
				       GA::GlobalArray* w,
				       double (* kernel)( const double& ) ) {
	...
	Kk->destroy();
	krig::KrigResult Result = { z, k, K, MSE, nullptr, krig::KrigType::simple };
	return Result;
    }
    else {
	GA_array galist = { x0, X, Z, w };
	throw KrigError( krig::KrigError::type, galist );
	krig::KrigResult Result = { nullptr, nullptr, nullptr, nullptr, nullptr, 
				    krig::KrigType::simple };
	return Result;
    }
}
...
/*__________________________________________________________________________________________________
  Ordinary Kriging
*/
krig::KrigResult krig::ordinary_kriging( GA::GlobalArray* x0, 
					 GA::GlobalArray* X,
					 GA::GlobalArray* Z,
					 GA::GlobalArray* w,
					 double (* kernel)( const double& ) ) {
	...
	Kk->destroy();
	ones->destroy();
	krig::KrigResult Result = { z, k, K, MSE, nullptr, krig::KrigType::ordinary };
	return Result;
    }
    else {
	GA_array galist = { x0, X, Z, w };
